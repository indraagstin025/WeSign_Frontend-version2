      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-6">
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-black text-zinc-900 dark:text-white tracking-tight">Panel Kontrol</h3>
          </div>
          <p className="text-[11px] text-zinc-500 dark:text-zinc-400 font-medium leading-relaxed px-1">
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-2">
              {signatures.map((sig, idx) => (
                ...
                >
                  <span className="w-6 h-6 rounded bg-white dark:bg-zinc-800 flex items-center justify-center text-[10px] font-bold text-zinc-400 border border-zinc-100 dark:border-white/10">
                    {idx + 1}
                  </span>
                  ...
                  <button 
                    ...
                    className="p-1.5 text-zinc-300 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-lg border-none bg-transparent cursor-pointer transition-colors"
                    title="Hapus"
                  >
                    <Trash2 size={14} />
                  </button>
        ...
        {signatures.length === 0 && !currentSignature && (
          <div className="flex-1 flex flex-col items-center justify-center text-center py-12 px-4 opacity-40">
            <div className="w-12 h-12 rounded-full border-2 border-dashed border-zinc-300 mb-3 flex items-center justify-center text-zinc-300">
               <FileText size={20} />
            </div>
            <p className="text-[10px] font-bold text-zinc-400 uppercase">Belum ada tanda tangan</p>
          </div>
        )}
      </div>
