...
const MobileBottomSheet = ({ 
  ...
  return (
    <>
      ...
      >
        {/* Sheet Container */}
        <div className="bg-white dark:bg-[#111b21] rounded-t-3xl shadow-2xl flex flex-col overflow-hidden border-t border-x border-slate-200 dark:border-white/10">
          ...
          <div className="px-5 pb-4 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-base font-black text-slate-900 dark:text-white tracking-tight text-left">Panel Kontrol</h3>
            </div>
            <button
              onClick={onClose}
              className="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-white/5 text-slate-400 border-none bg-transparent cursor-pointer transition-colors"
            >
              <X size={18} />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-5 pb-4 space-y-4" style={{ maxHeight: 'calc(85vh - 180px)' }}>
              ...
              <div className="space-y-2 text-left">
                ...
                <div className="space-y-2">
                  {signatures.map((sig, idx) => (
                    ...
                    >
                      <span className="w-7 h-7 rounded-lg bg-white dark:bg-slate-800 flex items-center justify-center text-xs font-bold text-slate-400 border border-slate-100 dark:border-white/10">
                        {idx + 1}
                      </span>
                      ...
                      <button 
                        ...
                        className="p-2 text-slate-300 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-lg border-none bg-transparent cursor-pointer transition-colors active:scale-90"
                        title="Hapus"
                      >
                        <Trash2 size={16} />
                      </button>
            ...
            {signatures.length === 0 && !currentSignature && (
              <div className="flex flex-col items-center justify-center text-center py-8 px-4 opacity-40">
                <div className="w-14 h-14 rounded-full border-2 border-dashed border-slate-300 mb-3 flex items-center justify-center text-slate-300">
                   <FileText size={22} />
                </div>
                <p className="text-xs font-bold text-slate-400 uppercase">Belum ada tanda tangan</p>
                <p className="text-[10px] text-slate-400 mt-1">Tap "Tambah Tanda Tangan" untuk memulai</p>
              </div>
            )}
          </div>
