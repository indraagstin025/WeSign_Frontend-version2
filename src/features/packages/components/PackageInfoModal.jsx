        ...
        <div className="relative w-full max-w-2xl bg-white dark:bg-slate-900 rounded-3xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 flex flex-col max-h-[90vh]">
          ...
          <div className="px-6 py-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between shrink-0 bg-slate-50/50 dark:bg-slate-900/50">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold text-slate-900 dark:text-white font-heading tracking-tight text-left flex items-center gap-2">
                  Detail Paket
                  <button 
                    onClick={() => {
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400 text-left">Informasi dan isi dokumen di dalam antrean paket</p>
              </div>
            </div>
            <button 
              onClick={handleClose} 
          ...
          <div className="flex-1 overflow-y-auto w-full p-6">
            {loading ? (
                <div className="flex flex-col items-center justify-center py-20 space-y-4">
                    ...
                    <p className="text-sm font-semibold text-slate-600 dark:text-slate-400">Memuat isi paket...</p>
                </div>
            ) : error ? (
              ...
              <div className="space-y-6">
                ...
                <div className="bg-slate-50 dark:bg-slate-800/50 p-5 rounded-2xl border border-slate-200 dark:border-slate-700/80">
                  <div className="flex justify-between items-start mb-4">
                        ...
                        {isEditing ? (
                           <div className="space-y-3">
                              <input 
                                ...
                                className="w-full px-4 py-2 bg-white dark:bg-slate-900 border border-primary/30 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 text-sm font-bold text-slate-900 dark:text-white"
                                placeholder="Judul Paket"
                              />
                              <select 
                                value={editCategory}
                                onChange={(e) => setEditCategory(e.target.value)}
                                className="w-full px-4 py-2 bg-white dark:bg-slate-900 border border-primary/30 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 text-sm text-slate-700 dark:text-slate-300 cursor-pointer"
                              >
                          ...
                          <>
                            <h2 className="text-xl font-black text-slate-900 dark:text-white leading-tight">{details.title || 'Tanpa Judul'}</h2>
                            <div className="flex items-center gap-2 mt-2">
                                ...
                                <span className="text-xs font-bold text-slate-600 dark:text-slate-400 bg-slate-200 dark:bg-slate-700 px-2 py-1 rounded-md">{details.label || 'Umum'}</span>
                            </div>
                          </>
                        )}
                      </div>
                      ...
                      {/* Edit Button */}
                      {!isEditing ? (
                          <button onClick={() => setIsEditing(true)} title="Ubah Nama Paket" className="p-2 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-600 transition-colors shadow-sm text-slate-500 dark:text-slate-300">
                             <PenTool size={16} />
                          </button>
                      ) : (
                          <div className="flex gap-2">
                              {saving ? (
                                  <div className="p-2 border border-slate-200 rounded-xl"><Loader2 size={16} className="animate-spin text-primary" /></div>
                              ) : (
                                  <>
                                    ...
                                    <button onClick={() => setIsEditing(false)} className="p-2 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-600 transition-colors shadow-sm text-slate-500"><XOctagon size={16} /></button>
                                  </>
                              )}
                          </div>
                      )}
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-4 border-t border-slate-200 dark:border-slate-700">
                      <div>
                          ...
                          <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">{new Date(details.createdAt).toLocaleDateString('id-ID', {day: 'numeric', month: 'long', year: 'numeric'})}</p>
                      </div>
                      <div>
                          <p className="text-[10px] uppercase font-bold text-slate-400 mb-1">Total Dokumen</p>
                          <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">{details.documents?.length || 0} File PDF</p>
                      </div>
                  </div>
                </div>
                ...
                {/* Sub-Documents List */}
                <div>
                   <h4 className="text-sm font-bold text-slate-800 dark:text-slate-200 border-b border-slate-200 dark:border-slate-800 pb-2 mb-3">Isi Dokumen di dalam Paket</h4>
                   <div className="space-y-3">
                       {details.documents?.map((item, idx) => {
                           const doc = item.docVersion?.document;
                           return (
                               <div key={item.id} className="flex gap-3 p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm items-center hover:border-primary/30 transition-colors">
                                  ...
                                  <div className="flex-1 min-w-0">
                                      <p className="text-sm font-bold text-slate-800 dark:text-slate-200 truncate">{doc?.title || 'Dokumen Tidak Diketahui'}</p>
                                      <p className="text-[11px] text-slate-500 truncate mt-0.5 font-mono">ID: {item.docVersionId.split('-')[0]}</p>
                                  </div>
                                  <div className="flex items-center gap-2">
                                    <button 
                                      onClick={() => navigate(`/dashboard/documents/preview/${doc?.id}`, { state: { from: '/dashboard/packages' } })}
                                      className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 dark:text-slate-500 transition-colors border-none bg-transparent cursor-pointer"
          ...
          {/* Footer Actions */}
          <div className="p-6 border-t border-slate-100 dark:border-slate-800 flex justify-between gap-4 bg-slate-50/50 dark:bg-slate-900/50 rounded-b-3xl">
              <button 
