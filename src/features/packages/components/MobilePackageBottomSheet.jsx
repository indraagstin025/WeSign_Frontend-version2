...
const MobilePackageBottomSheet = ({ 
  ...
  return (
    <>
      ...
      >
        <div className="bg-white dark:bg-[#111b21] rounded-t-[2.5rem] shadow-2xl flex flex-col overflow-hidden border-t border-x border-slate-200 dark:border-white/10">
          ...
          <div className="flex-1 overflow-y-auto px-6 pb-6 space-y-6 no-scrollbar" style={{ maxHeight: 'calc(90vh - 220px)' }}>
            ...
            {(activeTab === 'docs' || isReadOnly) && (
              <div className="space-y-3">
                <div className="flex items-center justify-between px-1">
                   <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">Pilih Dokumen</p>
                    ...
                    return (
                      ...
                      >
                        <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0
                          ${isActive ? 'bg-emerald-500 text-white' : 'bg-white dark:bg-slate-800 text-slate-400'}
                        `}>
                          {hasSignatures ? <CheckCircle2 size={18} /> : <span className="text-xs font-black">{idx + 1}</span>}
                        </div>
                        <div className="flex-1 min-w-0">
                           <p className={`text-xs font-bold truncate ${isActive ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-700 dark:text-slate-300'}`}>
                             {doc.docVersion?.document?.title || `Dokumen ${idx + 1}`}
                           </p>
                           <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tight mt-0.5">PDF Document</p>
                        </div>
            ...
            {activeTab === 'tools' && !isReadOnly && (
              <div className="space-y-6">
                ...
                {currentSignature && (
                  <div className="space-y-3">
                    <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] px-1">Tanda Tangan Aktif</p>
                    <div className="w-full aspect-[2.8/1] bg-white rounded-3xl border border-slate-200 dark:border-white/10 flex items-center justify-center p-5 shadow-inner overflow-hidden">
                ...
                {signatures.length > 0 && (
                  <div className="space-y-3">
                    <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] px-1">Ditempatkan Di Dokumen Ini ({signatures.length})</p>
                    <div className="space-y-2">
                      {signatures.map((sig, idx) => (
                        <div 
                          key={sig.id} 
                          className="flex items-center gap-3 p-4 rounded-2xl bg-white dark:bg-white/5 border border-slate-100 dark:border-white/5 transition-all shadow-sm"
                        >
                          ...
                          <button 
                            ...
                            className="p-2.5 text-slate-300 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10 rounded-xl border-none bg-transparent cursor-pointer transition-colors active:scale-90"
                          >
                            <Trash2 size={18} />
                          </button>
                        </div>
                ...
                {signatures.length === 0 && !currentSignature && (
                  <div className="flex flex-col items-center justify-center text-center py-10 opacity-30">
                    <div className="w-16 h-16 rounded-full border-2 border-dashed border-slate-300 mb-4 flex items-center justify-center text-slate-400">
                       <FileText size={28} />
                    </div>
                    <p className="text-xs font-black text-slate-400 uppercase tracking-widest">Belum ada Tanda Tangan</p>
                    <p className="text-[10px] text-slate-400 mt-2 font-bold px-8">Silakan buat tanda tangan baru dan tempelkan pada area dokumen.</p>
                  </div>
                )}
              </div>
            )}
          ...
          {/* Fixed Bottom Action */}
          <div className="p-6 bg-slate-50 dark:bg-[#1a2329] border-t border-slate-200 dark:border-white/5">
            <button 
