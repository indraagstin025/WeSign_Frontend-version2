...
const PackagePreviewPage = () => {
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center space-y-4 z-[100]">
        <div className="w-16 h-16 border-4 border-primary/20 border-t-primary rounded-full animate-spin" />
        <p className="text-xs font-black text-slate-500 uppercase tracking-widest animate-pulse">Memuat Pratinjau Paket...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="fixed inset-0 bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center p-6 text-center z-[100]">
        <AlertCircle size={48} className="text-rose-500 mb-4" />
        <h3 className="text-lg font-bold text-slate-900 dark:text-white">Gagal Memuat Paket</h3>
  ...
  return (
    <div className="fixed inset-0 z-[150] bg-slate-100 dark:bg-[#0b141a] flex flex-col overflow-hidden animate-in fade-in duration-300">
      
      {/* 1. TOP NAVBAR (Simplified for Preview) */}
      <div className="h-20 bg-white dark:bg-[#111b21] border-b border-slate-200 dark:border-white/5 flex items-center justify-between px-6 shrink-0 z-50">
          <div className="flex items-center gap-4">
              <button 
                onClick={() => navigate('/dashboard/packages')}
                className="p-2.5 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 transition-colors border-none bg-transparent cursor-pointer"
              >
                  <X size={20} />
              </button>
              <div className="h-6 w-px bg-slate-200 dark:bg-white/10" />
              <div>
                  <h1 className="text-sm font-black text-slate-900 dark:text-white leading-none mb-1 uppercase tracking-tight">
          ...
          <div className="flex items-center gap-3">
              <button 
                ...
                className="flex items-center gap-2 px-4 py-2.5 bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-300 transition-all border-none cursor-pointer"
              >
                  <Download size={16} />
      ...
      <div className="flex-1 flex overflow-hidden relative min-h-0">
        ...
        <main className="flex-1 overflow-y-scroll no-scrollbar bg-slate-100 dark:bg-[#0b141a] p-4 sm:p-8 flex items-start justify-center relative select-none pb-8 min-w-0" ref={containerRef}>
          <div 
          ...
          >
            ...
            <div className={`absolute inset-0 bg-white/90 dark:bg-[#0b141a]/95 backdrop-blur-[6px] z-[60] transition-all duration-700 ease-in-out ${ (pdfLoading || isRendering) ? 'opacity-100' : 'opacity-0 pointer-events-none' }`}>
                <div className="sticky top-0 h-[60vh] flex flex-col items-center justify-center">
                  ...
                  <p className="text-[10px] items-center font-black text-slate-500 dark:text-slate-400 uppercase tracking-[0.3em] animate-pulse pr-2 text-center">Menyiapkan Dokumen...</p>
                </div>
            </div>
