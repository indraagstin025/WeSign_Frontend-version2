    ...
    <div className="flex-1 overflow-y-auto no-scrollbar bg-zinc-50 dark:bg-zinc-950">
      <div className="max-w-7xl mx-auto px-5 lg:px-8 py-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
          <div>
            <h1 className="text-2xl font-bold text-zinc-900 dark:text-white">Paket Tanda Tangan</h1>
          ...
          >
            <Plus size={15} strokeWidth={2.5} /> Buat Paket Baru
          </button>
        </div>
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-2xl shadow-sm overflow-hidden">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 gap-4">
              ...
              <p className="text-[11px] font-semibold text-zinc-400 uppercase tracking-widest">Memuat paket...</p>
            </div>
          ) : error ? (
        ...
        {/* ── PAGINATION — hanya muncul jika total > PAGE_SIZE ──────── */}
        {showPagination && (
          <div className="flex items-center justify-between mt-4 px-1">
            <p className="text-[11px] text-zinc-400">
              Menampilkan {(currentPage - 1) * PAGE_SIZE + 1} - {Math.min(currentPage * PAGE_SIZE, total)} dari {total} paket
            </p>
            <div className="flex items-center gap-1">
              ...
              {getPageNumbers().map((p, i) =>
                p === '...' ? (
                  <span key={`ellipsis-${i}`} className="text-zinc-300 text-[11px] px-1">...</span>
                ) : (
                  <button
                    key={p}
                    onClick={() => goToPage(p)}
      ...
      />
      <EditPackageModal
        isOpen={modals.edit?.data !== null}
        pkg={modals.edit?.data}
