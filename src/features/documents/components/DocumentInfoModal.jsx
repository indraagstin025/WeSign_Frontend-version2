...
const DocumentInfoModal = ({ isOpen, onClose, document, onViewFile, onDownload }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 drop-shadow-2xl animate-in fade-in duration-300">
      ...
      {/* Modal Content */}
      <div className="relative w-full max-w-3xl bg-white dark:bg-slate-900 rounded-3xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-300 flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/50 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-900 dark:text-white font-heading tracking-tight">Informasi Dokumen</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 truncate max-w-[200px] sm:max-w-md">{document.title}</p>
            </div>
          </div>
          <button 
            onClick={onClose} 
            className="p-2 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-400 dark:text-slate-500 transition-colors cursor-pointer border-none bg-transparent"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h4 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider">Daftar Penanda Tangan</h4>
            </div>
            
            {document.signerRequests && document.signerRequests.length > 0 ? (
              <div className="overflow-hidden rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-800/20">
                <table className="w-full text-left text-xs sm:text-sm border-collapse">
                  <thead className="bg-slate-50 dark:bg-slate-800/50">
                    <tr>
                      <th className="px-4 py-3 font-bold text-slate-500 dark:text-slate-400">Nama / Email</th>
                      <th className="px-4 py-3 font-bold text-slate-500 dark:text-slate-400">Status</th>
                    </tr>
                  </thead>
                      ...
                      <tr key={req.id}>
                        <td className="px-4 py-3">
                          <div className="flex flex-col">
                            <span className="font-bold text-slate-900 dark:text-white">{req.user?.name || 'Unknown User'}</span>
                            <span className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-500">{req.user?.email}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          {req.status === 'SIGNED' ? (
              ...
              <div className="p-6 text-center bg-slate-50 dark:bg-slate-800/30 rounded-2xl border border-dashed border-slate-200 dark:border-slate-700">
                <p className="text-xs text-slate-500 dark:text-slate-500 italic">Dokumen ini bersifat pribadi & belum dibagikan untuk penandatanganan.</p>
              </div>
            )}
          </div>
          ...
          {document.versions && document.versions.length > 1 && (
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h4 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider">Riwayat Versi</h4>
              </div>
        ...
        {/* Footer Actions */}
        <div className="px-6 py-5 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 shrink-0 flex flex-col sm:flex-row gap-3">
          <button 
          ...
          <button 
            onClick={onClose}
            className="px-8 py-3 text-sm font-bold text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-all border-none bg-transparent cursor-pointer"
          >
            Tutup
          </button>
        </div>
...
const InfoRow = ({ icon, label, value, technical = false }) => (
  <div className="flex flex-col gap-1">
    ...
    <div className={`text-sm font-semibold text-slate-900 dark:text-slate-200 truncate ${technical ? 'font-mono text-[10px] text-slate-400 break-all whitespace-normal' : ''}`}>
      {value}
    </div>
  </div>
);
