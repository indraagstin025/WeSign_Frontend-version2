...
const VersionHistoryModal = ({ isOpen, onClose, document, onRollbackSuccess }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4">
      <div 
        className="w-full max-w-2xl bg-white dark:bg-slate-900 rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200"
      ...
      >
        ...
        <div className="flex items-center justify-between p-5 sm:p-6 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
          <div className="text-left">
            <h2 className="text-xl font-bold font-heading tracking-tight text-slate-800 dark:text-white">
          ...
          <button 
            onClick={onClose}
            className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-500 transition-colors border-none bg-transparent cursor-pointer"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <div className="p-5 sm:p-6 flex-1 max-h-[70vh] overflow-y-auto">
          {state.loading ? (
             <div className="flex flex-col items-center justify-center py-10">
               <div className="w-10 h-10 border-4 border-slate-100 dark:border-slate-800 border-t-primary rounded-full animate-spin" />
               <p className="mt-4 text-sm font-medium text-slate-500">Memuat riwayat...</p>
             </div>
          ) : state.error ? (
            ...
            <div className="space-y-6 relative">
              {/* Timeline Connector */}
              <div className="absolute left-[27px] top-8 bottom-8 w-0.5 bg-slate-200 dark:bg-slate-800" />
                ...
                return (
                  <div key={ver.id} className="relative pl-16">
                    ...
                    <div className="absolute left-0 top-1 w-14 flex justify-center">
                       <div className={`w-10 h-10 rounded-full flex items-center justify-center border-[3px] border-white dark:border-slate-900 z-10 ${
                    ...
                    <div className={`p-4 rounded-xl border text-left ${
                      isFinal ? 'bg-emerald-50/50 border-emerald-100 dark:bg-emerald-900/10 dark:border-emerald-800/30' : 'bg-white border-slate-200 dark:bg-slate-800 dark:border-slate-700'
                    }`}>
                       <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                         ...
                         <div className="flex flex-col gap-3 w-full sm:w-48 shrink-0 mt-4 sm:mt-0">
                           <button 
                             ...
                             className={`group flex items-center justify-center gap-2 w-full px-4 py-2.5 rounded-xl text-sm font-bold transition-all duration-200 border-none cursor-pointer active:scale-95 focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-slate-900 ${
                               isFinal 
                               ? 'bg-emerald-600 hover:bg-emerald-700 text-white shadow-md shadow-emerald-600/20 focus:ring-emerald-500' 
                               : 'bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-blue-400 dark:hover:border-blue-500 hover:text-blue-600 dark:hover:text-blue-400 text-slate-700 dark:text-slate-200 shadow-sm focus:ring-blue-500'
                             }`}
                           >
             ...
             <div className="text-center py-10 text-slate-500">Tidak ada riwayat.</div>
          )}
        </div>
      </div>
    </div>
