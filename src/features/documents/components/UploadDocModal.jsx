...
const UploadDocModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 drop-shadow-2xl animate-in fade-in duration-300">
      <div className="absolute inset-0 bg-slate-900/60 dark:bg-black/80 backdrop-blur-sm" onClick={actions.handleClose} />
            ...
            <>
              {!state.file ? (
                <div 
                  ...
                  className={`group relative border-2 border-dashed ${state.error ? 'border-rose-300 bg-rose-50/30' : 'border-slate-200 dark:border-slate-700'} rounded-2xl p-10 flex flex-col items-center justify-center gap-3 hover:bg-primary/5 hover:border-primary/50 transition-all cursor-pointer`}
                >
                  <div className={`w-16 h-16 ${state.error ? 'bg-rose-100 text-rose-500' : 'bg-slate-50 dark:bg-slate-800 text-slate-400'} rounded-full flex items-center justify-center group-hover:text-primary group-hover:scale-110 transition-all shadow-inner`}>
                    {state.validating ? <Loader2 size={32} className="animate-spin text-primary" /> : <FileBox size={32} />}
                  </div>
                  <div className="text-center">
                    <p className="text-sm font-bold text-slate-700 dark:text-slate-200">{state.validating ? 'Memvalidasi PDF...' : 'Klik atau Seret file PDF'}</p>
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-4 p-4 bg-primary/5 dark:bg-primary/10 rounded-2xl border border-primary/20 animate-in slide-in-from-bottom-2 duration-300">
                    ...
                    <div className="flex-1 min-w-0 pr-8 text-left">
                       <p className="text-sm font-bold text-slate-800 dark:text-slate-200 truncate">{state.file.name}</p>
                       <p className="text-[10px] text-slate-500 font-medium mt-0.5">{(state.file.size / 1024 / 1024).toFixed(2)} MB • PDF Digital</p>
                    </div>
                    {!state.loading && (
                      <button 
                        type="button"
                  ...
                  {state.loading && state.uploadProgress > 0 && (
                    <div className="space-y-2 px-1">
                      <div className="flex justify-between text-[11px] font-bold uppercase tracking-wider">
                        ...
                        <span className="text-slate-500">{state.uploadProgress}%</span>
                      </div>
              ...
              <div className="space-y-4">
                <div className="text-left">
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5 ml-1">Judul Dokumen</label>
                  <input 
                    type="text"
                    required
                    ...
                    className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 dark:text-white placeholder:text-slate-400 transition-all font-medium"
                  />
                </div>
                <div className="text-left">
                  <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5 ml-1">Kategori / Tipe</label>
                  <select 
                    disabled={state.loading}
                    value={state.type}
                    onChange={(e) => actions.setType(e.target.value)}
                    className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 dark:text-white transition-all cursor-pointer font-medium"
                  >
